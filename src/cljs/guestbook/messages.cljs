 ...
 (fn [{:keys [db]} [_ author]]
   {:db       (-> db
                  (assoc :messages/loading? true
                         :messages/list nil
                         :messages/filter {:author author})) ;; Adding a filter to only add
    ;; messages of the author when received from websockets
    :ajax/get {:url           (str "/api/messages/by/" author)
               :success-path  [:messages]
               :success-event [:messages/set]}}))


;; Set current message to the provided input
(rf/reg-event-db
 :messages/set
 (fn [db [_ messages]]
   (-> db
...
;; Check for a list of filters if msg satisfy every predicate of them
;; Predicates can be values, sets or function
